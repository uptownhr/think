  ...
  <div id="option-tree-slider-editor_<?php echo $count; ?>" class="option-tree-slider">
    ...
    <div class="option-tree-slider-body">
      ...
      foreach( $image_slider_fields as $field ) {
        ...
        } /* from here is all rb */ else if($field['type'] == 'checkbox'){
			echo '
			  <p>
				<label>'.$field['label'].'</label>
				<input data-actions="'.$field['data-actions'].'" type="checkbox" name="'.$id.'['.$count.']['.$field['name'].']" value="'.( isset($field['name']) ? stripslashes($field['name']) : '' ).'" class="'.$field['class'].'" '.checked(stripslashes($field['name']),true,false).' />
			</p>';
		}
      }
      ?>
